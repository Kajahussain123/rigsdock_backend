...
exports.filterProductsByBrand = async (req, res) => {
  try {
    ...
    if (products.length === 0) {
      return res.status(404).json({ message: "No products found for the selected brand" });
    }

    res.status(200).json({ message: "Products filtered by brand successfully", products });
  } catch (error) {
    res.status(500).json({ message: "Error filtering products by brand", error: error.message });
  }
};
...
exports.filterProductsByPriceRange = async (req, res) => {
  try {
    ...
    if (!minPrice && !maxPrice) {
      return res.status(400).json({ message: "At least one price filter (minPrice or maxPrice) is required" });
    }
    ...
    const products = await Product.find(filter).populate("maincategory category subcategory brand");

    if (products.length === 0) {
      return res.status(404).json({ message: "No products found in the given price range" });
    }

    res.status(200).json({ message: "Products filtered by price range successfully", products });
  } catch (error) {
    res.status(500).json({ message: "Error filtering products by price range", error: error.message });
  }
};

exports.filterProductsByRating = async (req, res) => {
  try {
    const { minRating } = req.query;

    if (!minRating) {
      return res.status(400).json({ message: "Minimum rating is required" });
    }

    const allProducts = await Product.find().populate("maincategory category subcategory brand");
    ...
    for (let product of allProducts) {
      ...
      if (averageRating >= Number(minRating)) {
    ...
    if (filteredProducts.length === 0) {
      return res.status(404).json({ message: "No products found with the given rating filter" });
    }

    res.status(200).json({ message: "Products filtered by rating successfully", products: filteredProducts });
  } catch (error) {
    res.status(500).json({ message: "Error filtering products by rating", error: error.message });
  }
};
